...
%>
<form name="offer" action="offer1.jsp" method="post" > 
...
<center>
<table><tr><td><div class="tab"> Description</div></td>
        <td><div class="tab">Amount</div></td>
        <td><div class="tab">Select</div></td>
    </tr>
<% 
while(rs.next()){
	...
	if(rs.getString("operator").compareTo(operator)==0 /* && rs.getString("state").compareTo(state)==0 */)
		...
		flag++;
		%>
		<tr><td><div class="tab"><%=offerdetail %></div></td><td><div class="tab"><%=amount %></div></td>
                    <td  class="tab"> <a href="session.jsp?id=<%=rs.getString("id")%>">select</a></td>
                </tr>
		<% 
		}
